...
int hash_index(char *hash_this)
{
    ...
    return hash % HASHTABLE_SIZE; // No need for abs() as the result is already unsigned
}
...
bool load(const char *dictionary)
{
    ...
    // Scans dictionary word by word (populating hash table with nodes containing words found in
    // dictionary)
    char word[LENGTH + 1];
...
bool check(const char *word)
{
    ...
    // If the word exists, you should be able to find it in the dictionary data structure.
    ...
    // Cursor has reached end of list and word has not been found in dictionary so it must be
    // misspelled
    return false;
}
...
bool unload(void)
{
    // Loop over all buckets in hashtable
    for (int i = 0; i < HASHTABLE_SIZE; i++)
    {
        node *cursor = hashtable[i];

        // Traverse and free linked list at each bucket
        while (cursor != NULL)
        {
            node *temp = cursor;
            cursor = cursor->next;
